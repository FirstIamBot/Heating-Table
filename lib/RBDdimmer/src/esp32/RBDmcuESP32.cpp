...
// Tunable TRIAC gate timings for ESP32 implementation.
// Increase pulse width if the TRIAC does not latch reliably near zero-cross.
static const uint16_t TRIAC_PULSE_TICKS = 6; // 4 6 8
static const uint16_t TRIAC_FULL_POWER_DELAY_TICKS = 0;

int pulseWidth = TRIAC_PULSE_TICKS;
...
{	
	...
	if (power == 100)
	{
		dimPulseBegin[this->current_num] = TRIAC_FULL_POWER_DELAY_TICKS;
	}
	else
	{
